...
async function loadCampaignInfo() {
  try {
    ...
    if (data.success) {
      campaignInfo = data.campaigns.find(c => c.id == currentCampaignId);
      if (campaignInfo) {
        renderCampaignInfo();
      }
    }
...
function renderCampaignInfo() {
  ...
  container.innerHTML = `
    <div class="campaign-name">${campaignInfo.name}</div>
    <div class="campaign-description">${campaignInfo.description || 'No description available'}</div>
    <div style="color: #aaa; font-size: 14px;">Campaign ID: ${campaignInfo.id}</div>
  `;
}
...
async function loadPermissions() {
  try {
    const response = await fetch(`/api/campaign_permissions?campaign_id=${currentCampaignId}`);
    const data = await response.json();
    if (data.success) {
...
function renderPermissions() {
  ...
  permissions.forEach(permission => {
    ...
    const avatarLetter = permission.username.charAt(0).toUpperCase();
    
    div.innerHTML = `
      <div class="user-info">
        <div class="user-avatar">${avatarLetter}</div>
        <div class="user-details">
          <h4>${permission.username}</h4>
