...
let currentCampaignIdScenes = null;
let selectedSceneId = null;
let activeSceneId = null;
let scenes = [];
let campaignInfoScenes = null;
// Get campaign ID from session storage
function getCampaignIdFromSessionScenes() {
...
async function loadScenes() {
    try {
        const response = await fetch(`/api/scenes?campaign_id=${currentCampaignIdScenes}`);
        const data = await response.json();
...
if (createSceneForm) {
    createSceneForm.addEventListener('submit', async function (e) {
        e.preventDefault();
        ...
        const gridSizeInput = document.getElementById('scene-grid-size');
        const backgroundColorInput = document.getElementById('scene-background-color');
...
window.openCreateSceneModal = openCreateSceneModal;
window.closeCreateSceneModal = closeCreateSceneModal;
window.selectScene = selectScene;
window.activateScene = activateScene;
window.editScene = editScene;
window.openSceneMap = openSceneMap;
