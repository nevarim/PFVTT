import { Journal, JournalsResponse, ApiResponse } from '../types/shared.js';

// Additional interfaces specific to journals
interface CreateJournalResponse extends ApiResponse {
  journal?: Journal;
}
...
let currentCampaignIdJournals: string | null = null;

// Get campaign ID from session storage
function getCampaignIdFromSessionJournals(): string | null {
...
function renderJournalList(): void {
...
function selectJournal(journalId: number): void {
...
function showJournalSheet(journal: Journal): void {
...
function openCreateModal(): void {
  const modal = document.getElementById('createJournalModal') as HTMLElement;
  const titleInput = document.getElementById('journalTitle') as HTMLInputElement;
  const contentInput = document.getElementById('journalContent') as HTMLTextAreaElement;
...
if (createJournalForm) {
  createJournalForm.addEventListener('submit', async function(e: Event) {
    e.preventDefault();
...
function editJournal(journalId: number): void {
  alert('Edit functionality not yet implemented');
}

// Delete journal
async function deleteJournal(journalId: number): Promise<void> {
...
(window as any).selectJournal = selectJournal;
(window as any).editJournal = editJournal;
